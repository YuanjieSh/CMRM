...
from train.corrupted_cifar import corrupted_CIFAR10, corrupted_CIFAR100
from train.corrupted_mini import corrupted_MiniImageNet
...
parser.add_argument('--alpha', default=0.1, type=float, help='Desired nominal marginal coverage')
...
# parser.add_argument('-ar', '--arch', metavar='ARCH', default='resnet20',
#                     choices=model_names,
...
parser.add_argument('--tr_alpha', default=0.1, type=float,
                    metavar='tr_alpha', help='tr_alpha for cvar', dest='tr_alpha')
...
parser.add_argument('--t_gap', type=float, default='0.0',
                    help='Concentration gap of truncated')
parser.add_argument('--c_gap', type=float, default='0.0',
                    help='Concentration gap of classwise')
parser.add_argument('--cl_gap', type=float, default='0.0',
...
parser.add_argument('--lambda_val', type=float, default= 0.15, 
                    help='hyper-parameter for regularization')
parser.add_argument('--aug', default = 'RandomErasing', type = str, help = 'corruption method')
...
parser.add_argument('--tr_samples', default = 3000, type = int, help = 'number of data points used for training')
parser.add_argument('--model', default = 'resnet20', type = str, help = 'model architecture')
...
def find_real_batch_size_dir(base_dir, batch_size):
    for d in os.listdir(base_dir):
        if d.startswith(f"batch-size={batch_size}"):
            return d
    raise FileNotFoundError(f"No batch-size dir with prefix batch-size={batch_size} found in {base_dir}")


if args.loss_type == 'CE_reg' or args.loss_type == 'Focal_reg' or args.loss_type == 'LDAM_reg':
    # base_path = "dataset={}/architecture={}/loss_type={}/noise_rho={}/reg={}/alpha={}/train_rule={}/start_epochs=100/epochs={}/batch-size={}\
    #     /lr={}/momentum={}/".format(args.dataset, args.model, args.loss_type, args.noise_rho, args.reg, args.tr_alpha, args.train_rule,\
    #         args.epochs, args.batch_size, args.lr, args.momentum)
    base_dir = os.path.join(
        os.getcwd(),
        "checkpoint/noise/fine_tune",
        f"dataset={args.dataset}",
        f"architecture={args.model}",
        f"loss_type={args.loss_type}",
        f"noise_rho={args.noise_rho}",
        f"reg={args.reg}",
        f"alpha={args.tr_alpha}",
        f"train_rule={args.train_rule}",
        f"start_epochs={args.start_epoch}",
        f"epochs={args.epochs}"
    )

    batch_dir = find_real_batch_size_dir(base_dir, args.batch_size)

    store_name = '_'.join([
        args.dataset, args.model, args.loss_type, args.train_rule,
        str(args.noise_rho), args.exp_str, str(args.reg), str(args.tr_alpha)
    ])
    patha_model = os.path.join(base_dir, batch_dir, f"lr={args.lr}", f"momentum={args.momentum}", store_name, "ckpt.pth.tar")
    patha = './Results/' + os.path.join(base_dir, batch_dir, f"lr={args.lr}", f"momentum={args.momentum}")

else:
    base_path = "dataset={}/architecture={}/loss_type={}/noise_rho={}/train_rule={}/epochs={}/batch-size={}\
        /lr={}/momentum={}/".format(args.dataset, args.model, args.loss_type, args.noise_rho, args.train_rule,\
             args.epochs, args.batch_size, args.lr, args.momentum)
    store_name = '_'.join([args.dataset, args.model, args.loss_type, args.train_rule, str(args.noise_rho), args.exp_str])
    patha_model = '/checkpoint/noise/' + base_path + store_name + '/ckpt.best.pth.tar'
    patha_model = os.getcwd() + patha_model
    # print(patha_model)
    patha = './Results/' + base_path 
...
if dataset == "cifar10":
 ...
 #   print(num_train_samples[0], num_train_samples[2], num_train_samples[3], num_train_samples[5], num_train_samples[6], num_train_samples[9])
    ...
    train_dataset = corrupted_MiniImageNet(
    ...
    val_dataset = corrupted_MiniImageNet(
            root_dir='data/mini-imagenet',
...
# dir_path = patha
# filename_2 = 'acc_vector.npy'
# checkpoint_path_2 = os.path.join(dir_path, filename_2)

# acc_vector = load_or_compute_acc_vector(softmax_scores, labels, num_classes, checkpoint_path_2)
# acc_vector = np.array(acc_vector)

def run_one_experiment(softmax_scores, labels, dataset, save_folder, num_classes, alpha, bins, truncated_gap, class_gap, cluster_gap, k_r, lam, n_totalcal, score_function_list, methods, seeds, 
    ...
    for score_function in score_function_list:
        ...
        for seed in seeds:
            ...
            if score_function == 'HPS':
                ...
                val_scores_all = 1 - val_scores
            elif score_function == 'APS':
                if method == 'cluster_CP':
                    ...
                    totalcal_scores_all = get_APS_scores(totalcal_scores, totalcal_labels, seed)
                    # scores_median = np.median(totalcal_scores_all)
                    # scores_mean = np.mean(totalcal_scores_all)
                    # print(scores_median)
                    # print(scores_mean)
                    val_scores_all = get_APS_scores_all(val_scores, seed)
            elif score_function == 'RAPS': 
                # RAPS hyperparameters (currently using ImageNet defaults)
                lmbda = lam 
                kreg = k_r
            ...
            for method in methods:
                ...
                if method == 'MCP':
                    ...
                    all_results[method] = truncated_classwise_conformal(accuracy_matrix, totalcal_scores_all, totalcal_labels, totalcal_ranks, 
                                                               val_scores_all, val_labels, val_rank, alpha, 
                                                               num_classes=val_scores_all.shape[1],
                                                               default_qhat=np.inf, regularize=False)
                elif method == 'k-CCP_T':
                    # Classwise conformal  
            ...
            if not save_preds:
                for m in all_results.keys():
                    all_results[m] = (all_results[m][0], all_results[m][1], all_results[m][2], all_results[m][3])
...
def initialize_metrics_dict(methods):
    ...
    for method in methods:
        metrics[method] = {
                           # 'Under Coverage Ratio': [],
                           'Avg Set Size': [],
                           # 'avg_set_size': [],
                           'marginal_cov': [],
                           # 'Avg Set Size': []
                           } # Could also retrieve other metrics
...
def average_results_across_seeds(folder, print_results=True, show_seed_ct=False, 
    ...
    for pth in file_names:
        ...
        for method in methods:
            try:
                ...
                metrics[method]['Avg Set Size'].append(results[method][3]['mean'])
    ...
    for method in methods:
        ...
        marginal_cov_std.append(np.std(metrics[method]['marginal_cov'])/np.sqrt(n))
        ...
        if print_results:
            print(f"  {method}:"
                  f" marginal_cov_mean: {np.mean(metrics[method]['marginal_cov'])}", 
                  f"marginal_cov_std: {np.std(metrics[method]['marginal_cov'])/np.sqrt(n)}", 
    ...
    df = pd.DataFrame({'method': methods,
                      ...
                      'marginal_cov_std': marginal_cov_std})
...
# run_one_experiment(softmax_scores = softmax_scores_2, labels = labels_2, dataset = args.dataset, save_folder = patha, num_classes = num_classes, alpha = args.alpha, bins = args.bins, truncated_gap = args.t_gap, class_gap = args.c_gap, cluster_gap = args.cl_gap, lam = args.lmbda_val, k_r = args.k_reg, n_totalcal = args.avg_num_per_class, score_function_list = args.score_functions, methods = args.methods, seeds = args.seeds, 
#                        cluster_args={'frac_clustering':'auto', 'num_clusters':'auto'},
...
if args.all == 'yes':
    ...
    # print(result_folder)
    table_results = average_results_across_seeds(folder = result_folder, print_results=True, show_seed_ct=True, methods=['MCP'], max_seeds=np.inf)
    output_path = os.path.join(patha, f'{args.dataset}_{args.model}_loss_{args.loss_type}_score_{args.score_functions}_total_results.csv')
