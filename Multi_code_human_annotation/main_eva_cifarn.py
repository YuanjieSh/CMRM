...
def load_dataset(args, dataset, data_folder='CCC_data'):
    '''
    ...
    if args.loss_type == 'CE':
        data = np.load(f'{data_folder}/{dataset}_{args.noise_mode}.npz')
    elif args.loss_type == 'CR2M':
        data = np.load(f'{data_folder}/{dataset}_{args.noise_mode}_cr2m.npz')
    else:
...
base_path = "dataset={}/noise_mode={}/loss_type".format(args.dataset, args.noise_mode, args.loss_type)
patha = '.AISTATS/'+ base_path
patha_2 = '.AISTATS/'+ base_path
if not os.path.exists(patha):
    os.makedirs(patha)
# load datasets

softmax_scores, labels = load_dataset(args, dataset)
...
run_one_experiment(softmax_scores = softmax_scores, labels = labels, dataset = args.dataset, save_folder = patha, num_classes = num_classes, alpha = args.alpha, bins = args.bins, truncated_gap = args.t_gap, class_gap = args.c_gap, cluster_gap = args.cl_gap, lam = args.lmbda_val, k_r = args.k_reg, n_totalcal = args.avg_num_per_class, score_function_list = args.score_functions, methods = args.methods, seeds = args.seeds, 
                       cluster_args={'frac_clustering':'auto', 'num_clusters':'auto'},
